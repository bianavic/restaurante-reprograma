...
fetch('http://localhost:3001/comidas')
  ...
  .then((comidas) =>{
    comidas.forEach(prato => {
      const mediaItem = document.createElement('div'); // media item é cada item da comida
      ...
      <div class="media-body>
        ...
        buttonDelete.setAttribute("class", "btn btn-info btn-sm")
        buttonDelete.setAttribute("data-id", prato._id)
        mediaItem.appendChild(buttonDelete)

        buttonDelete.addEventListener("click", () =>{
          fetch(
            `http://localhost:3001/comidas/${prato._id}`,
            {
              method: 'DELETE',
              headers: {
